...
erDiagram
    ...
    places ||--o{ banners : "banner_link"
    
    banners ||--o{ ad_clicks_impressions : "banner_events"
...
| `province_id` | `integer` | `Int` | `FK`, `onDelete: Restrict` | Liên kết `provinces(id)` |
| `category_id` | `integer` | `Int` | `FK`, `onDelete: Restrict` | Liên kết `categories(id)` |
| `provider_id` | `integer` | `Int?` | `FK`, `onDelete: SetNull` | Đối tác sở hữu (`providers`) |
...
| `price_range` | `varchar(100)` | `String?` | | Khoảng giá (VD: 20k - 100k) |
| `has_parking` | `boolean` | `Boolean` | `default: false` | Có bãi đỗ xe không |
| `is_sponsored` | `boolean` | `Boolean` | `default: false` | Được tài trợ (đẩy nổi bật) hay không |
| `sponsored_expires_at`| `timestamptz` | `DateTime?` | | Thời điểm hết hạn gói tài trợ |
...
*   **Chỉ mục (Indexes)**:
    ...
    - `idx_places_province`: B-Tree trên cột `province_id`.
    - `idx_places_category`: B-Tree trên cột `category_id`.
    - `idx_places_provider`: B-Tree trên cột `provider_id`.
...
### 15. Bảng `banners`
Thông tin các Banner quảng bá trang chủ (Liên kết đối tác quảng cáo).
...
| `type` | `varchar(20)` | `String` | | Loại banner (ví dụ: `home`, `place`) |
...
### 16. Bảng `ad_clicks_impressions` [NEW]
...
| :--- | :--- | :--- | :--- | :--- |
| `id` | `bigint` | `BigInt` | `PK`, `autoincrement()` | Khóa chính |
| `banner_id` | `integer` | `Int?` | `FK`, `onDelete: Cascade` | Liên kết banner cụ thể |
