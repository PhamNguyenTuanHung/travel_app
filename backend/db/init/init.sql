...
-- Bảng Banner & Khuyến mại (Đã chỉnh sửa thêm cột provider_id, place_id và CHECK dates)
CREATE TABLE banners (
    id SERIAL PRIMARY KEY,
    provider_id INT REFERENCES providers(id) ON DELETE CASCADE, -- Nullable cho banner nội bộ hệ thống
    ...
    CONSTRAINT chk_banner_dates CHECK (start_date <= end_date)
);

-- Bảng log click & hiển thị quảng cáo
...
CREATE INDEX idx_banners_provider ON banners(provider_id);
CREATE INDEX idx_banners_place ON banners(place_id);
CREATE INDEX idx_ad_logs_banner ON ad_clicks_impressions(banner_id);
...
-- =============================================================================
-- 4. CHÈN BẢNG ĐỊA ĐIỂM (PLACES) - Đã liên kết với provider_id
-- =============================================================================
INSERT INTO places (
